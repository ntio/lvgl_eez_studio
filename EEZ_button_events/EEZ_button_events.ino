...
void setup() {
  ...
  lv_display_t *disp;
  disp = lv_tft_espi_create(SCREEN_WIDTH, SCREEN_HEIGHT, draw_buf, DRAW_BUF_SIZE);//TFT_HOR_RES, TFT_VER_RES,
  ...
  //Add event for the 4 buttons.
  lv_obj_add_event_cb(objects.btn_clicked, button_click_event_handler, LV_EVENT_CLICKED, NULL);
  lv_obj_add_event_cb(objects.btn_long_clicked, button_long_click_event_handler, LV_EVENT_LONG_PRESSED, objects.label_click_result);
...
/**
* If label isn't passed into the event than it will update label with warning message.
...
static void button_change_colour_event_handler(lv_event_t *e) {
  ...
  //Generate random colour and set button background
